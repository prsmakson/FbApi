...
namespace Fb.Api.Models.Exstension
{
    public static class ClassExtensions
    {
        ...
        {
            string request = account.GetBaseUri() + "me/" + "?fields=businesses&access_token=" + account.getToken();
        ...
        {
            string request = account.GetBaseUri() + business.id + "?fields=owned_ad_accounts{name,account_id,account_status}&access_token=" + account.getToken();
            var jText = RequestHelper.SendGetRequest(request);
        ...
        {
            string request = account.GetBaseUri() + business.id + "?fields=owned_pages{name,id}&access_token=" + account.getToken();
            var jText = RequestHelper.SendGetRequest(request);
        ...
        public static IEnumerable<Pixel> GetPixels(this Business business, Account account)
        {
            ...
            string request = account.GetBaseUri() + business.id + "?fields=owned_pixels{name,id}&access_token=" + account.getToken();
            var jText = RequestHelper.SendGetRequest(request);
